...
function artistSearchHome(artist){
    fetch(`http://localhost:3000/homeArtist/${artist}`)  // the root 
    .then(response => response.json())
    .then(songs => {
        let para = "";
        let input = "";
...
map.on("click", e=>{
    ...
    if(text.length <= 2 && town.length <= 2){
        ...
        fetch(`http://localhost:3000/artistAdd/${text}/${lattitude}/${longtitude}/${town}`,{  // inserting the details to the database
            method: 'POST',   // this is needed otherwise the browser would do the default get method 
        })
        .then(response => response.json())
        .then(data => {
        ...
        L.marker(markerLet).addTo(map); // adding it to the map 
...
function getAllMarkers() {
    ...
    fetch(`http://localhost:3000/getAllMarkers`)
    .then(response => response.json())
    .then(markers => {
